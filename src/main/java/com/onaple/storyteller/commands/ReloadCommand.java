...
public class ReloadCommand implements CommandExecutor {
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
        ...
        try {
            int dialogCount = Storyteller.getInstance().loadConfig();
            src.sendMessage(
                    Text.builder("Dialogues configuration reloaded.").color(TextColors.GREEN)
                    .append(Text.builder(" " + dialogCount + " ").color(TextColors.GOLD).build())
                    .append( Text.builder("dialogues loaded.").color(TextColors.GREEN).build()).build()
            );

            Storyteller.getLogger().info("Reloaded storyteller configuration with " + dialogCount + " dialogs.");
